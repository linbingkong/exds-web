...
def _has_real_trade_review(target_date: str) -> bool:
    if DATABASE.day_ahead_energy_declare.count_documents({"date_str": target_date}, limit=1) > 0:
        return True
    start_dt = datetime.strptime(target_date, "%Y-%m-%d")
    end_dt = start_dt + timedelta(days=1)
    return DATABASE.day_ahead_energy_declare.count_documents(
...
def get_next_day_simulation(trade_source_id: str = Query(...), _ctx: CurrentUserContext = Depends(require_permission(VIEW_PERMISSION))) -> SimulationDetailModel:
    ...
    if not result_doc:
        return _simulation_response(trade_source, _build_empty_manual_result_doc(trade_source, target_date), "未申报")
    return _simulation_response(trade_source, result_doc, "已申报")
