...
- 若页面有页头、筛选栏或工具栏，页头用 `flex: '0 0 auto'`，主体区用 `flex: '1 1 0'` 和 `minHeight: 0`
- 主体区不要再叠加 `height: '100%'`，否则容易把页头高度也算进去，导致页面级纵向滚动条
...
- 桌面端卡片层通常需要 `overflow: 'hidden'`，移动端回到 `overflow: 'visible'`
- 真正很长的列表或表格外层使用 `flex: '1 1 0'`、`minHeight: 0`、`overflowY: 'auto'`
- 不要用固定 `maxHeight` 作为桌面端满高页的主要滚动边界，除非该区域本来就不参与剩余高度分配
...
2. 页面出现纵向滚动条：
- 通常是多层 `padding + gap + minHeight` 叠加，或主体区在页头下面又写了 `height: '100%'`，把总高度撑破。
...
5. 某个面板很长并撑出页面底部：
- 通常是卡片内部的长列表、表格或权限矩阵没有自己的 `flex: '1 1 0' + minHeight: 0 + overflowY: 'auto'` 滚动边界。
